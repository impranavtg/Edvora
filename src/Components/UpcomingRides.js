...
const UpcomingRides = () => {
  ...
  return (
    ...
    {allupcomingrides.length===0?(<div style={{display:"flex",justifyContent:"center",alignItems:"center"}}><img style={{width:"30%"}} src={notfound} alt="Rides Not Found" /></div>):allupcomingrides.map((r)=>(<Ride key={r.id+r.origin_station_code} rideInfo={r}/>))}
    </div>
  )
}
